...
module Zones =
    ...
    type Location() =
        ...
        member this.EdgeUpGroundParse (tiles:Tile[][]) =        
            let fromGrounds = this.Grounds |> Seq.collect(fun x -> x.Cells)
                                            |> Seq.append (this.Platforms |> Seq.collect(fun x -> x.Cells))
                                     |> Seq.map(fun x -> seq { x.toLeftTop; x.toRightTop})
                                     |> Seq.collect(fun x -> x)
            ...
            let toGrounds = this.Grounds |> Seq.collect(fun x -> x.Cells)
                                                |> Seq.append (this.Platforms |> Seq.collect(fun x -> x.Cells))
                                     |> Seq.map(fun x -> seq { x.toLeftTop; x.toMidTop; x.toRightTop})
                                     |> Seq.collect(fun x -> x)
            ...
            cross
                |> Seq.filter(fun (a,b) -> a.Y < b.Y && (Vector2.dist a b) <= maxJump)
                |> Seq.choose(fun (a,b) ->
        ...
        member this.buildPathMap (tiles:Tile[][])  =
            ...
            pathMap <- pathsUp |> Seq.map(fun (a,b) -> 
                                                    Cell.fromVector a,
                                                    {
